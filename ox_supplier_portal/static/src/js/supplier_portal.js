odoo.define('ox_supplier_portal.SupplierRegister', function (require) {
    'use strict';
    ...
    publicWidget.registry.s_website_form_supplier = publicWidget.Widget.extend({
        selector: '.s_website_form_supplier form, form.s_website_form_supplier',
        events: {
            'click .s_website_form_send_supplier, .o_website_form_send_supplier': 'send',
            'submit': 'send',
        },
        ...
        send: async function (e) {
            ...
            if(window.location.pathname === '/my/supplier/register'){
                let formId = this.$target.attr('id')
                this._listReferences(formId)   
            }

            // Prevent users from crazy clicking
            const $button = this.$target.find('.s_website_form_send_supplier, .o_website_form_send');
            ...
            ajax.post(this.$target.attr('action') + (this.$target.data('force_action') || this.$target.data('model_name')), form_values)
                .then(async function (result_data) {
                    ...
                    self.$target.find('.s_website_form_send_supplier, .o_website_form_send')
                        .removeAttr('disabled')
        ...
        update_status: function (status, message) {
            if (status !== 'success') { // Restore send button behavior if result is an error
                this.$target.find('.s_website_form_send_supplier, .o_website_form_send')
                    .removeAttr('disabled')
                    .removeClass('disabled'); // !compatibility
            ...
            if(status !== 'error'){
                let formId = this.$target.attr('id')
                this._toggleCollapseAccordion(formId)
            }
        ...
        _toggleCollapseAccordion(formId) {

            formId = parseInt(formId.replace('frp', '')) - 1

            setTimeout(function(){
                $('#myCollapse .collapse').eq(formId).collapse('toggle');
            }, 3000);
        },

        _listReferences(formId) {

            formId = parseInt(formId.replace('frp', ''))
            const $refData = $('[name="ref_data"]');
            ...
            if(formId == 4){
                ...
                let jsonData = {}
                let references = {}
                let columns = ["ref_name", "ref_type", "ref_cargo", "ref_street", "ref_mobile", "ref_email", "ref_res"]

                $("#tbReference > tbody").children("tr").each(function() {

                    $(this).children("td").each(function(){

                        if(counter < 6){
                            let columnName = columns[counter];
                            jsonData[columnName] = $(this).html();
                        }

                        if(counter == 6){
                            $(this).children("input").each(function(){
                                let columnName = columns[counter];
                                jsonData[columnName] = $(this).val();
                            });
                        }
                        counter++;
                    });

                    references[counter_data] = jsonData
                    jsonData = {}
                    counter = 0;
                    counter_data++;
                });

                $refData.val(JSON.stringify(references));
            }
        }

    });

    publicWidget.registry.of_city_change = publicWidget.Widget.extend({

        selector: '[name="state_id"]', // !compatibility
        events: {
            'change': 'send',
        },
        ...
        send: async function (e) {
            ...
            const $button = $('[name="state_id"]')
            const $buttonCity = $('[name="city_id"]')

            $.ajax({ 
                url:'/portal/city/' + $button.val(),
                type: 'GET',
                'dataType': "json",
                success:function(response)
                {
                    $buttonCity.find('option').remove().end();
    ...
    publicWidget.registry.of_btn_add_reference_click = publicWidget.Widget.extend({

        selector: '#btnAddReference',
        events: {
            'click': 'add',
        },
        ...
        add: function (e) {
            ...
            const $refName = $('[name="ref_name"]');
            const $refType = $('[name="ref_type"]');
            const $refCargo = $('[name="ref_cargo"]');
            const $refStreet = $('[name="ref_street"]');
            const $refMobile = $('[name="ref_mobile"]');
            const $refEmail = $('[name="ref_email"]');

            const $tbReference = $('#tbReference tbody');

            let newRowContent = '<tr><th scope="row">1</th><td>' + $refName.val() + '</td><td>' + $refType.val() + '</td><td>' + $refCargo.val() + '</td><td>' + $refStreet.val() + '</td><td>' + $refMobile.val() + '</td><td>' + $refEmail.val() + '</td><td><a href="#" name="deleteRow" class="btn btn-primary fa fa-close" /><input type="hidden" value="0" /></td></tr>';

            $tbReference.append(newRowContent);

            $refName.val('');
            $refType.val('Referencia Personal');
            $refCargo.val('');
            $refStreet.val('');
            $refMobile.val('');
            $refEmail.val('');
